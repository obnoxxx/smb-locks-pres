...
\hline
 ...
 others &  per client request \\
\hline
\end{tabular}
\end{center}
\end{block}
\begin{alertblock}{Warning: very coarse}<3->

\textbf{cum grano salis -- details differ}
\end{alertblock}
\begin{alertblock}{Warning: relevance ...}

what NFS clients use which calls?

\end{alertblock}
\begin{alertblock}{TODO: Multi-Protocol}<4->
\begin{itemize}
\item investigate NFS clients' use of delegations/resetvations
...
\begin{frame}{Multi-Proto SMB+NFS}{possible approach}



\pause
\begin{block}{client OS}<2->
Identify relevant client operating systems, e. g.:
\begin{itemize}
\item MS windows
\item MacOS
\item Linux/Unix
...
\pause
\begin{block}{Apps/syscalls}a<3-
identify relevant applications, e. g.:
\begin{itemize}
\item MS office
\item libreoffice
\item database
\item git / development 
\end{itemize}
identify  syscalls related to storage/locking used by apps?
\end{block}
% -> which syscalls?
\pause
\begin{block}{protocol clients / calls}<4->
identify relevant protocol clients, e. g.:
\begin{itemize}
\item  Windows smb mount (\texttt{net use})
\item  Windows nfs client??
\item Linux kernel smb client (\texttt{cifs.ko}) ??
\item Linux kernel nfs client
\item MacOS smb client
\item macOS nfs client??
\end{itemize}
identify (mapped) file protocol calls used 
\end{block} 

\begin{block}{reference server}<5->
    udentify reference server, e. g.:
    \begin{itemize}
    \item MS Windows
    \item netapp
    \item EMC isilon
    ...
    \end{itemize}
\end{block}
\begin{block}{test suite}<6->
    develop  client test suite for contention  against reference server 
\end{block}

\begin{block}{server implementation}<7->
  model multi-proto server behavior after testsuite
\end{block}
